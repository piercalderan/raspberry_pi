# Esempio di gioco fatto con Python e la lbreria PyGame + suoni.
...
pygame.mixer.pre_init(44100, -16, 2, 2048) # setup del mixer
pygame.init()

suono_topo = pygame.mixer.Sound("mouse.wav")  # carica il primo fileWAV
suono_fine = pygame.mixer.Sound("crash.wav")  # carica il secondo fileWAV
...
while True:
	...
	for evento in pygame.event.get():
		if evento.type == QUIT:
			...
			if evento.key == K_UP and direzione != 0:direzione = 2;
			elif evento.key == K_DOWN and direzione != 2:direzione = 0
			elif evento.key == K_LEFT and direzione != 1:direzione = 3
			elif evento.key == K_RIGHT and direzione != 3:direzione = 1
	i = len(xs)-1
	while i >= 2:
		...
		i-= 1
	if collisione(xs[0], topo_pos[0], ys[0], topo_pos[1], 20, 10, 20, 10):punteggio+=1;xs.append(700);ys.append(700);topo_pos=(random.randint(0,590),random.randint(0,590));suono_topo.play()
